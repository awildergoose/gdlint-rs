use crate::{
    gdscript::{
        symbols::{
            ...
            BitXorDoubleQuote, BitXorEq, Colon, ColonEq, Comma, Div, DivEq, Dollar, Dot, DotDotDot,
            DoubleQuote, Eq, EqEq, Gt, GtEq, GtGt, GtGtEq, Hashregion, LBrace, LBracket, LParen,
            LParenRParen, Lt, LtEq, LtLt, LtLtEq, Mod, ModEq, Mul, MulEq, MulMul, MulMulEq,
            Not as NotSymbol, NotEq, Or as OrSymbol, OrEq, OrOr, RBrace, RBracket, RParen,
            Semicolon, Sub, SubEq, SubGt,
        },
        unnamed::{
            And, As, Await, Break, Class, ClassName, Const, Continue, Elif, Else, Enum, Export,
            Extends, For, Func, Get, If, In, Init, Is, Master, Mastersync, Match, Not, Onready, Or,
            Pass, Puppet, Puppetsync, Remote, Remotesync, Return, Set, Signal, Value, Var, When,
            While,
        },
        Annotation, Annotations, Arguments, Array, Assignment, Attribute, AttributeCall,
        AttributeSubscript, AugmentedAssignment, AwaitExpression, BaseCall, BinaryOperator, Body,
        BreakStatement, BreakpointStatement, Call, ClassBody, ClassDefinition, ClassNameStatement,
        Comment, ConditionalExpression, ConstStatement, ConstructorDefinition, ContinueStatement,
        DefaultParameter, Dictionary, ElifClause, ElseClause, EnumDefinition, Enumerator,
        EnumeratorList, EscapeSequence, ExportVariableStatement, ExpressionStatement,
        ExtendsStatement, False, Float, ForStatement, FunctionDefinition, GetBody, GetNode, Getter,
        Identifier, IfStatement, InferredType, Integer, Lambda, LineContinuation, MatchBody,
        ...
        TypedDefaultParameter, TypedParameter, UnaryOperator, VariableStatement, VariadicParameter,
        WhileStatement,
    },
    Traverser,
};
...
impl Traverser {
    pub fn visit_g_d_string(&self, node: GDString) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_conditional_expression(&self, node: ConditionalExpression) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_constructor_definition(&self, node: ConstructorDefinition) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_parameters_(&self, node: Parameters_) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_typed_default_parameter(&self, node: TypedDefaultParameter) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_and_symbol(&self, node: AndSymbol) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_not_symbol(&self, node: NotSymbol) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn visit_or_symbol(&self, node: OrSymbol) -> anyhow::Result<()> {
        Ok(())
    }
